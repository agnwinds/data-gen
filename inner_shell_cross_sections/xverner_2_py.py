...
History:
15aug    nsh    Coded
20mar    ksl    This is an updated version of verner_2_py.py
                that attempt to deal with the fact the VKFY
                does not contain all of the atoms of interest
                specifically, P.  It produeces and adititional
                file to handle these extra x-sections, namely
                vy_outershell_tab.data
...
for i in range(len(data1['z'])):
    ...
    #Set up an energy array. Start just before threshold to make sure we get the threshold
    #Set all the cross sections to zero
    e=np.logspace(np.log10(data1['eth'][i]+0.00001),np.log10(emax),npoints)  
    csec=np.zeros(len(e))    
    ...
    #We now have the cross sections up to the limit of the VFKY data - search for relevant data in the VY data file to append
    #to the x-section we have already found in vkfy.

    for j in range(len(data2['z'])): 
        if     data2['z'][j]==data1['z'][i] and data2['nelec'][j]==data1['nelec'][i]: #We have the correct element
            ...
            for ii in range(len(outer_shell_list)):  #We have found the correct list of outer shells, loop over them
                if data2['n'][j]==outer_shell_list[ii][0] and data2['l'][j]==outer_shell_list[ii][1]: 
                    #If the n and l for the current VY record, use it
                    data2['Used'][j]='v'
...
for i in range(len(data2['z'])):
    #Generate the energy array, running from the threshold to the global maximum energy
    e=np.logspace(np.log10(data2['eth'][i]+0.00001),np.log10(emax),npoints)   
    csec=np.zeros(len(e))   
    ...
    if outer==0:  
        #If we got through the whole check without changing the flag, this must be an inner shell of an ion that we understand
        for k in range(len(e)):  #Run over all energies
            ...
            csec[k]=csec[k]+data2['sigma0'][i] * temp    
            data2['Used'][i]='i'
            #This is the cross section, we will now append to all out arrays
        energies.append(e)
...
data2.write('foo.txt',format='ascii.fixed_width_two_line',overwrite=True)
#output the arrays.
...
# Now handle the rest of the VY data.  This should be outershell data which is not part of 
# of VFKY
xdata=data2[data2['Used']=='n']
print('Test',len(xdata))
...
for i in range(len(xdata)):
    #Generate the energy array, running from the threshold to the global maximum energy
    e=np.logspace(np.log10(xdata['eth'][i]+0.00001),np.log10(emax),npoints)   
    csec=np.zeros(len(e))   

    for k in range(len(e)):  #Run over all energies
        y=e[k]/xdata['e0'][i]
        temp=(y-1.0)**2.0+xdata['yw'][i]**2.0
        q=5.5+xdata['l'][i]-0.5*xdata['P'][i]
        temp=temp*y**(-1.0*q)
        temp=temp*(1+np.sqrt(y/xdata['ya'][i]))**(-1.0*xdata['P'][i])
        csec[k]=csec[k]+xdata['sigma0'][i] * temp    
        xdata['Used'][i]='i'
        #This is the cross section, we will now append to all out arrays
    energies.append(e)
    cross_secs.append(csec)
    z.append(xdata['z'][i])
    nelec.append(xdata['nelec'][i])
    n.append(xdata['n'][i])
    l.append(xdata['l'][i])

#Output the arrays as a python friendly file
out1=open("vy_outershell_tab.data",'w')
#Write out some preamble
out1.write("# These are x-sections from VY95, that are not part of VKFY (1996) \n")
out1.write("# It is processed via the python script verner_2_py.py from simple text dumps of the online\n")
out1.write("# data from these two papers\n")
out1.write("#\n")
out1.write("# The data is read in using the topbase mechanics, so level and level_label are just placholders\n")
...
for i in range(len(xdata)):
    istate=xdata['z'][i]-xdata['nelec'][i]+1
    out1.write('PhotVfkyS %d %d %d %d %5.3e %d\n'%(xdata['z'][i],istate,-999,-999,xdata['eth'][i],npoints))
    for j in range(npoints):
        out1.write("PhotVfky %7.4e %7.4e\n"%(energies[i][j],cross_secs[i][j]*1e-18))
out1.close()
